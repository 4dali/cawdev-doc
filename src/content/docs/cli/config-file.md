...
| `agentCommands` | string[] | `["claude"]` | Which coding agent(s) this machine spawns for a run — `claude` (Claude Code), `agy` (Antigravity CLI) and/or `codex` (Codex CLI), told apart by the command name itself. `CAWDEV_AGENT_COMMAND` (comma-separated) overrides it. |
...
| `autoWorkspaces` | number | `0` | How many checkouts per project this machine may **clone by itself** when a coding run would otherwise wait for a free one — named `<project>-auto-<n>`, found again by name on restart, removed by [`/drop`](/cli/terminal-ui/#advanced-mode) in the attached terminal. `0` means none: a clone spends disk and time, so it's switched on, never on by default. Hot-swappable — a running daemon picks up a change on `SIGHUP`, no restart needed. |
